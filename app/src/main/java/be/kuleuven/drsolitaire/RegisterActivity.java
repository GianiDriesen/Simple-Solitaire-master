...
public class RegisterActivity extends AppCompatActivity {
    ...
    public void registerApp(View view) {
        if (password.getText().toString().equals(password2.getText().toString())) {
            SharedData.getEntityMapper().getpMapper().getPersonByUsernameAndPassword(username.getText().toString(),password.getText().toString());
        ...
        protected void onPostExecute(Person person) {
            if (person != null) {
                ...
                getSharedPreferences(SharedData.PREFS_NAME,MODE_PRIVATE)
                        ...
                        .putString(SharedData.PREF_PASSWORD, password.getText().toString())
                        .commit();
